...
namespace ADService
{
    public partial class ADService : ServiceBase
    {
        ...
        public const uint READ_ATTRIBUTES = 0x00000080;
        ...
        private void onBeforeOpenFile(object Sender, CbfilterBeforeOpenFileEventArgs e)
        {
            ...
            Console.WriteLine("BeforeRead: {0} by process {1}. GR:{2,5}, A:{3,5}, RC:{4,5}, SY:{5,5}, DA:{6}, OPT:{7}, SM:{8}", fname, process, isGenericRead(e.DesiredAccess), isReadAttributes(e.DesiredAccess), isReadControl(e.DesiredAccess), isSynchronize(e.DesiredAccess), Convert.ToString(e.DesiredAccess, 2).PadLeft(32, '0'), Convert.ToString(e.Options, 2).PadLeft(32, '0'), Convert.ToString(e.ShareMode, 2).PadLeft(32, '0'));
            
            /*if (isGenericRead(e.DesiredAccess) && !(isReadControl(e.DesiredAccess) || isReadAttributes(e.DesiredAccess))) {
                e.ProcessRequest = false;
                e.ResultCode = (int)ERROR_PRIVILEGE_NOT_HELD;
                return;
            }*/

            if (isReadControl(e.DesiredAccess) || isReadAttributes(e.DesiredAccess))
            {
                return;
            }
            ...
            if (fsize >= 1024)
            {
                ...
                Console.WriteLine("BeforeOpen: " + fname + " >=1K. Process: " + process);
            }
            else
            {
                Console.WriteLine("BeforeOpen: " + fname + " <1K. Process: " + process);
        ...
        {
            return (flag & READ_ATTRIBUTES) == READ_ATTRIBUTES;
        }

        private bool isGenericRead(int flag)
        {
            return (flag & GENERIC_READ) == GENERIC_READ;
        }
